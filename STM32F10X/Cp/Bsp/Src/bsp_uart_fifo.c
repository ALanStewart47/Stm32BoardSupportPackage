...
//uint8_t g_uart1_rx_buffer[1];          /* HAL库使用的串口接收数据缓冲区 */
...
void comDmaSendBuf(COM_PORT_E _ucPort, uint8_t *_ucaBuf, uint16_t _usLen)
{
	...
	//HAL_UART_Transmit_DMA(&huart1, _ucaBuf, _usLen);
	UartSend(pUart, _ucaBuf, _usLen);
}
...
void comDmaSendChar(COM_PORT_E _ucPort, uint8_t _ucByte)
{
	comDmaSendBuf(_ucPort, &_ucByte, 1);
...
static void UartVarInit(void)
{
	...
	g_tUart3.SendBefor = 0;						/* RS485_SendBefor发送数据前的回调函数 */
	g_tUart3.SendOver = 0;						/* RS485_SendOver发送完毕后的回调函数 */
	g_tUart3.ReciveNew = 0;						/* RS485_ReciveNew接收到新数据后的回调函数 */
...
void bsp_SetUartParam(USART_TypeDef *Instance,  uint32_t BaudRate, uint32_t Parity, uint32_t Mode)
{
	...
	//HAL_UART_Receive_IT(&UartHandle,g_tUart1.pRxBuf,UART1_RX_BUF_SIZE);
}


//void bsp_SetUartDMAParam(USART_TypeDef *Instance,  uint32_t BaudRate, uint32_t Parity, uint32_t Mode)
//{
//	UART_HandleTypeDef UartHandle	= {0};
...
//	}
//	
////	SET_BIT(USART1->SR, USART_SR_TC);					/* 清除TC发送完成标志 */
////	SET_BIT(USART1->SR, USART_SR_RXNE);  			/* 清除RXNE接收标志 */
////	SET_BIT(USART1->CR1, UART_IT_RXNE);				/* 使能RX非空中断 */
//	
...
//    DmaTxHandle.Init.Priority = DMA_PRIORITY_HIGH;
//    if (HAL_DMA_Init(&DmaTxHandle) != HAL_OK)
//    {
//      Error_Handler();
//    }

//    __HAL_LINKDMA(&UartHandle,hdmatx,DmaTxHandle);
//		
...
static void InitHardUart(void)
{
  ...
  //GPIO_InitStruct.Pull = GPIO_PULLUP;
	...
	/* 配置波特率、奇偶校验 ，由于添加了波特率可调的功能，故固定值改为变量	*/
	bsp_SetUartParam(USART1,  g_Uart1_Baud, UART_PARITY_NONE, UART_MODE_TX_RX);	// UART_MODE_TX_RX
	...
	SET_BIT(USART2->SR, USART_SR_TC);	/* 清除TC发送完成标志 */
	SET_BIT(USART2->SR, USART_SR_RXNE);/* 清除RXNE接收标志 */
	SET_BIT(USART2->CR1, UART_IT_RXNE);	/* 使能PE. RX接受中断 */
	...
	SET_BIT(USART3->SR, USART_SR_TC);	/* 清除TC发送完成标志 */
	SET_BIT(USART3->SR, USART_SR_RXNE);/* 清除RXNE接收标志 */
	SET_BIT(USART3->CR1, UART_IT_RXNE);	/* 使能PE. RX接受中断 */
#endif
	...
	SET_BIT(UART4->SR, USART_SR_TC);	/* 清除TC发送完成标志 */
	SET_BIT(UART4->SR, USART_SR_RXNE);/* 清除RXNE接收标志 */
	SET_BIT(UART4->CR1, UART_IT_RXNE);	/* 使能RX接受中断 */
#endif
	...
	SET_BIT(UART5->SR, USART_SR_TC);	/* 清除TC发送完成标志 */
	SET_BIT(UART5->SR, USART_SR_RXNE);/* 清除RXNE接收标志 */
	SET_BIT(UART5->CR1, UART_IT_RXNE);	/* 使能RX接受中断 */
#endif
}
...
static void UartIRQ(UART_T *_pUart)
{
	...
	{	
		...
		//HAL_UART_Receive_IT(&huart1,(uint8_t *)g_uart1_rx_buffer,1);
...
//			}
	...
	/* 清除中断标志 */
	SET_BIT(_pUart->uart->SR, USART_SR_PE);
	SET_BIT(_pUart->uart->SR, USART_SR_FE);
	SET_BIT(_pUart->uart->SR, USART_SR_NE);
	SET_BIT(_pUart->uart->SR, USART_SR_ORE);
	SET_BIT(_pUart->uart->SR, USART_SR_IDLE);
	SET_BIT(_pUart->uart->SR, USART_SR_TC);
	SET_BIT(_pUart->uart->SR, USART_SR_LBD);
	SET_BIT(_pUart->uart->SR, USART_SR_CTS);
